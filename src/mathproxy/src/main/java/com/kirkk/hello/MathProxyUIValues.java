...
public class MathProxyUIValues implements Values {
	...
	public Map<String, Integer> getValues() {
		...
		try {
			...
			if (mathOp.equals("a")) {
				vals.put("type", MathProxy.ADD);
			} else if (mathOp.equals("s")) {
				vals.put("type", MathProxy.SUBTRACT);
			} else if (mathOp.equals("m")) {
				vals.put("type", MathProxy.MULTIPLY);
			} else {
				vals.put("type", MathProxy.DIVIDE);
			}
